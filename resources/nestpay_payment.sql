...
CREATE TABLE `nestpay_payment` (
  ...
  `trantype` varchar(7) NOT NULL COMMENT 'Transaction type Set to "Auth" for authorization, “PreAuth” for preauthorization',
  `amount` int(11) NOT NULL COMMENT 'amount transaction amount Use "." or "," as decimal separator, do not use grouping character',
  `currency` int(3) NOT NULL COMMENT 'ISO code of transaction currency ISO 4217 numeric currency code, 3 digits',
  `Response` varchar(9) DEFAULT NULL COMMENT 'Payment status. Possible values: "Approved", "Error", "Declined"',
  ...
  `mdStatus` int(1) DEFAULT NULL COMMENT 'Status code for the 3D transaction. 1=authenticated transaction 2, 3, 4 = Card not participating or attempt 5,6,7,8 = Authentication not available or system error 0 = Authentication failed',
  `ErrMsg` varchar(255) DEFAULT NULL COMMENT 'Error message',
  `AuthCode` char(6) DEFAULT NULL COMMENT 'Transaction Verification/Approval/Authoriza tion code',
  ...
  `BillToCountry` varchar(3) DEFAULT NULL COMMENT 'BillTo country code',
  ...
  `ShipToCountry` varchar(3) DEFAULT NULL COMMENT 'ShipTo country code',
  ...
  `instalment` int(3) NULL COMMENT 'Instalment count',
  `INVOICENUMBER` varchar(15) DEFAULT NULL COMMENT 'Invoice Number',
  `storetype` varchar(16) DEFAULT NULL COMMENT 'Merchant payment model Possible values: "pay_hosting", “3d_pay”, "3d", "3d_pay_hosting"',
  `lang` varchar(2) DEFAULT NULL COMMENT 'Language of the payment pages hosted by NestPay',
  `xid` char(28) DEFAULT NULL COMMENT 'Internet transaction identifier',
  `HostRefNum` char(12) DEFAULT NULL COMMENT 'Host reference number ',
  `ReturnOid` varchar(64) DEFAULT NULL COMMENT 'Returned order ID, must same as input orderId',
  `MaskedPan` char(12) DEFAULT NULL COMMENT 'Masked credit card number',
  `rnd` char(20) DEFAULT NULL COMMENT 'Random string, will be used for hash comparison',
  `merchantID` char(15) DEFAULT NULL COMMENT 'MPI merchant ID',
  `txstatus` char(1) DEFAULT NULL COMMENT '3D status for archival Possible values "A", "N", "Y"',
  `iReqCode` int(2) DEFAULT NULL COMMENT 'Code provided by ACS indicating data that is formatted correctly, but which invalidates the request. This element is included when business processing cannot be performed for some reason.',
  `iReqDetail` int(2) DEFAULT NULL COMMENT 'May identify the specific data elements that caused the Invalid Request Code (so never supplied if Invalid Request Code is omitted).',
  `vendorCode` char(50) DEFAULT NULL COMMENT 'Error message describing iReqDetail error.',
  `PAResSyntaxOK` char(1) DEFAULT NULL COMMENT 'If PARes validation is syntactically correct, the value is true. Otherwise value is false. "Y" or "N',
  `PAResVerified` char(1) DEFAULT NULL COMMENT 'If signature validation of the return message is successful, the value is true. If PARes message is not received or signature validation fails, the value is false. "Y" or "N',
  `eci` int(2) DEFAULT NULL COMMENT 'Electronic Commerce Indicator. empty for non-3D transactions',
  `cavv` char(28) DEFAULT NULL COMMENT 'Cardholder Authentication Verification Value, determined by ACS. 28 characters, contains a 20 byte value that has been Base64 encoded, giving a 28 byte result.',
  `cavvAlgorthm` int(1) DEFAULT NULL COMMENT 'CAVV algorithm Possible values "0", "1", "2", "3"',
  `md` char(10) DEFAULT NULL COMMENT 'MPI data replacing card number',
  `Version` char(3) DEFAULT NULL COMMENT 'MPI version information 3 characters l(ike "2.0")',
  `sID` int(1) DEFAULT NULL COMMENT 'Schema ID "1" for Visa, "2" for Mastercard',
  `mdErrorMsg` text DEFAULT NULL COMMENT 'Error Message from MPI (if any)',
  `clientid` varchar(50) DEFAULT NULL,
  `EXTRA_TRXDATE` char(17) DEFAULT NULL,
